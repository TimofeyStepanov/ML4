...
class LinearRegression:
    ...
    def fit(self, x: np.ndarray, y: np.ndarray) -> LinearRegression:
        ...
        for _ in range(self.max_iter):
            self.loss_history.append(self.calc_loss(x, y)) # значение функции потерь
            weigh_diff = self.descent.step(x, y)  # происходит шаг градиентного спуска

            # Евклидова норма или Nan
            if (np.linalg.norm(weigh_diff, 2) ** 2) <= self.tolerance or (sum(np.isnan(weigh_diff)) > 0):
                break
