...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if self.loss_function is LossFunction.MSE:
            ...
            #loss = (np.mean(np.log(1 + np.cosh(y - x @ self.w)))) # - тут одни inf
            loss = np.mean(self.logcosh(y - x @ self.w))
        else:
            raise NotImplementedError('BaseDescent calc_loss function not implemented')

        return loss
    
    def logcosh(self, x: np.ndarray) -> np.ndarray:
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        diff = - (self.lr() / (np.sqrt(v_) + self.eps)) * m_
        self.w = self.w + diff
